export const getProduct = async (_, { input }, context) => {
    try {
        const { id } = input;
        // query the user
        const products = await context.db.query(`SELECT p.id, image_url, name, description, price, quantity, seller_id,
        ...
        const reviews = await context.db.query(`SELECT r.id, rating, comment, ui.first_name || ' ' || ui.last_name as buyer_name from ratings r
