...
export const searchProducts = async (_: any, {input}: any, context: any) => {
    const {text} = input;
    try {
      ...
      const products = await context.db.query(
        "Select * from products p join product_inventory pi on p.id = pi.product_id where p.deleted_at is null and pi.quantity > 0 and p.name ilike '"+text+"%' LIMIT 10;", 
      [])
